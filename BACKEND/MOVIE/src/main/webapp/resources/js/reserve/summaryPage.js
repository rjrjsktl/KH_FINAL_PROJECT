    ...
    success: function (priceMap) {
      for (let i = 0; i <= priceMap.countArray.length; i++) {
        $("#price_calc > div")
          .eq(i)
          .find(".age_price")
          .html(priceMap.priceArray.at(i));
        $("#price_calc > div")
          .eq(i)
          .find(".age_pick")
          .html(priceMap.countArray.at(i));
      }
...
function updateScreenSection(userPlay) {
  ...
  for (let k = 1; k <= maxRow; k++) {
    $(`#seat_area > div:nth-child(${k}) > div:first-child`).text(
      alphabet[k - 1]
    );
  }
...
function plusAisle(r, c) {
  $(`#seat_area > div:nth-child(${r}) > a:nth-of-type(${c})`).removeClass(
    "seat"
  );
...
function plusSpace(r, c) {
  $(
    `#seat_area > div:nth-child(${r}) > a:not('.aisle'):nth-of-type(${c})`
  ).removeClass("seat");
  $(
    `#seat_area > div:nth-child(${r}) > a:not('.aisle'):nth-of-type(${c})`
  ).addClass("space");
...
$(".seat").on("click", function (e) {
  if (!$(this).hasClass("aisle") && !$(this).hasClass("space")) {
    seatNo =
      alphabet[$(this).closest("div").index()] +
      ($(this).prevAll(".seat, .space").length + 1);
...
for (let i = 0; i < priceArray.length; i++) {
  $(`#price_calc > div:nth-child(${i + 1}) > div > span:first-child`).text(
    priceArray[i]
  );
}
