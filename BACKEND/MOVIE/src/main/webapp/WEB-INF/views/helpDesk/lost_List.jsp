														...
														<tr class="row">
															...
															<c:choose>
																<c:when test="${lostList.lostPw eq 0}">
																	<td><a
																		href="${contextPath}/helpDesk/checkLostPw/${lostList.lostNo}?cp=${param.cp}"
																		id="mtmList_pass">${lostList.lostTitle}</a>
																		<c:if
																			test="${lostList.lostRepSt == 'Y'}"> <span class="replyComplete">.... [ 답변완료 ]</span></c:if>
																	</td>
																</c:when>
																<c:when
																	test="${lostList.lostPw eq 1111 and loginUser.userManagerSt eq 'Y'
